...
在上述代码中，如果T的数据类型传入的是像 Person 这样的自定义数据类型，类内没有重载 > 运算符，也无法正常运行
...
void test01()
{
	...
	// m.fun2();	// 编译会出错，说明函数调用才会去创建成员函数
    				// 函数本身有问题，Person1 调用不了 Person2 的 showPerson2，但是对象 m 不调用 fun2 ，
    				// 编译不出错，说明函数没调用就不会去创建成员函数
}
```
...
void test01()
{
	...
	string str3 = "I";
	str3.append(" love ");
	str3.append("game abcde", 4);	// 添加 "game"
	//str3.append(str2);
...
* `erase(const_iterator start, const_iterator end);`//删除迭代器从start到end之间的元素（包头不包尾）
* `clear();`                                                        //删除容器中所有元素
...
总结：sort算法非常实用，使用时包含头文件 **algorithm** 即可，内部无 sort 算法
...
* set / multiset 属于**关联式容器**，底层结构是用**红黑树**实现。
...
* 查找   ---  find    （返回的是迭代器）（成员函数）
* 统计   ---  count  （对于set，结果为0或者1）（成员函数）
...
* map/multimap属于**关联式容器**，底层结构是用**红黑树**实现。
