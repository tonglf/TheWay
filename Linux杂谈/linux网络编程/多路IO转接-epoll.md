...
epoll 通过 epoll_create 创建一棵红黑树和一个双端链表，利用 epoll_ctl 将感兴趣的事件挂在到红黑树上，并为每个事件创建回调函数（该过程发生在内核态），当有事件发生时，该事件调用回调函数，然后将感兴趣的事件放入双端链表中，直接获取双端链表的感兴趣事件即可进行通信，时间复杂度为 O(1)。
